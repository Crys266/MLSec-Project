...
from captum.attr import LayerIntegratedGradients, IntegratedGradients, InputXGradient
...
class IGAttributions(Attributions):
    def __init__(
        ...
        print(f"attr: input_ids type: {self.input_ids.dtype}")

        self.ig = IntegratedGradients(self.custom_forward)
        self._calculate_attributions()

    def _calculate_attributions(self):
        # Converti i tensori in LongTensor
        self.input_ids = self.input_ids.long()
        self.ref_input_ids = self.ref_input_ids.long()
        if self.token_type_ids is not None:
            self.token_type_ids = self.token_type_ids.long()
        if self.position_ids is not None:
            self.position_ids = self.position_ids.long()
        if self.ref_token_type_ids is not None:
            self.ref_token_type_ids = self.ref_token_type_ids.long()
        if self.ref_position_ids is not None:
            self.ref_position_ids = self.ref_position_ids.long()
        # Aggiungi stampe per verificare i tipi e i valori
        print(f"After conversion in IGAttributions - input_ids type: {self.input_ids.dtype}, values: {self.input_ids}")


        if self.token_type_ids is not None and self.position_ids is not None:
            self._attributions = self.ig.attribute(
                inputs=(self.input_ids, self.token_type_ids, self.position_ids),
                baselines=(
                    self.ref_input_ids,
                    self.ref_token_type_ids,
...
class InputXGradientAttributions(Attributions):
    def __init__(
        ...
        self.input_x_grad = InputXGradient(self.custom_forward)
        self._calculate_attributions()

    def _calculate_attributions(self):
        if self.token_type_ids is not None and self.position_ids is not None:
            self._attributions = self.input_x_grad.attribute(
                inputs=(self.input_ids, self.token_type_ids, self.position_ids),
                target=self.target,
            )
        elif self.position_ids is not None:
