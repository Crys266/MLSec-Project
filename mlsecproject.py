...
# --- Sezione LIME: spiegazione black-box modificata ---

# Metodo LIME
print("\nComputing black-box explanation with LIME...")

lime_explanations = []
normalized_aligned_text = []
...
# Definizione di una nuova funzione di tokenizzazione da usare con LIME.
# Questa funzione utilizza il tokenizer del modello per ottenere i token,
# ma rimuove i prefissi "##" in modo che i token risultanti siano sottostringhe che appartengono
...
for i, sentence in enumerate(sentences):
    ...
    # Salva direttamente in saved_explanations i risultati normalizzati
    lime_explanations.append({
         "sentence": sentence,
         "explanation": explanation,  # originale, nel caso sia ancora utile per altre operazioni
         "normalized_attributions": normalized_aligned,
...
for entry in lime_explanations:
    # Qui possiamo decidere se mostrare l'esempio originale o, ad esempio, stampare i token normalizzati.
    # Se il metodo show_in_notebook() è comunque necessario, possiamo richiamarlo sul campo 'explanation'
    entry["explanation"].show_in_notebook(text=entry["sentence"])
...
augmenter = Augmenter(transformation=transformation, pct_words_to_swap=0.2, transformations_per_example=10)
...
for i, text in enumerate(texts):
    ...
    print(f"🔥 Modified: \"{modified_versions}\" -> {modified_predictions} (Prob: {modified_probs[i][modified_predictions[i]]:.4f})")
...
for entry in ad_bb_attributions:
    print(f"\nAttributions for method: \"{entry['explain_type']}\"")
    print(entry['attributions'])
    tokens = [token for token, _ in entry["attributions"]]
    scores = [score for _, score in entry["attributions"]]
...
for entry in ad_wb_attributions:
    print(f"\nAttributions for method: \"{entry['explain_type']}\"")
    print(entry['attributions'])
    tokens = [token for token, _ in entry["attributions"]]
    scores = [score for _, score in entry["attributions"]]
